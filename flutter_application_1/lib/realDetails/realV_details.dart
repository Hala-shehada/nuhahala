...
class DetailV extends StatefulWidget {
  final rating1;
  ...
  @override
  State<DetailV> createState() => _DetailVState();
}

class _DetailVState extends State<DetailV> {
  double rating = 0;
  ...
  Future myComments() async {
    var url = Uri.parse("http://192.168.1.100/gradpro/comments.php");
    ...
    if (response.statusCode == 200) {
      ...
      if (jsonData.length != 0) {
        ...
        while (comments.length != i) {
          var url = Uri.parse("http://192.168.1.100/gradpro/userNameImage.php");
          var response = await http.post(url,
              headers: {"Accept": "application/json"},
    ...
    var url = Uri.parse('http://192.168.1.100/gradpro/totalCommentsInVilla.php');
  ...
  Future getLikes() async {
    var url = Uri.parse('http://192.168.1.100/gradpro/selectLikes.php');
    var response = await http.post(url, body: {
  ...
  Future addLike() async {
    var url = Uri.parse('http://192.168.1.100/gradpro/addLike.php');
    var response = await http.post(url, body: {
      "user_id": widget.login_id,
      "post_id": widget.post_id,
      "cat_type": widget.cat_type,
  ...
  Future addComments() async {
    var url = Uri.parse('http://192.168.1.100/gradpro/addComments.php');
    var response = await http.post(url, body: {
                    ...
                    children: [
                      ...
                      Padding(
                        ...
                        child: Row(
                          mainAxisAlignment: MainAxisAlignment.spaceBetween,
                          children: [
                            buildFeature(Icons.hotel, "Rooms ${widget.beadroom}"),
                            buildFeature(Icons.wc, "Bathrooms ${widget.bathroom}"),
                            buildFeature(Icons.living,"Halls ${widget.hall}" ),
  ...
  Widget buildFeature(IconData iconData, text) {
    return Column(
      children: [
        Icon(
          iconData,
...
class MyMapp1 extends StatefulWidget {
  final List firstlist;
  const MyMapp1({Key? key, required this.firstlist}) : super(key: key);

  @override
  State<MyMapp1> createState() => _MyMappState1();
}
// late double pos1;
// late double pos2;

class _MyMappState1 extends State<MyMapp1> {
  ...
  addNewMarker(LatLng latLng) {
    Marker marker = Marker(
      ...
      position: latLng,
      // mapController.animateCamera(CameraUpdate.newLatLngZoom(latLng, 25)),
    );
    markers.add(marker);
  ...
  @override
  void initState() {
    double pos1 = double.parse(widget.firstlist[0]);
    double pos2 = double.parse(widget.firstlist[1]);
    // ignore: no_leading_underscores_for_local_identifiers
    final LatLng _pos = LatLng(pos1, pos2);
    _center = _pos;

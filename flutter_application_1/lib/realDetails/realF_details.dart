...
class DetailF extends StatefulWidget {
  final rating1;
  ...
  @override
  State<DetailF> createState() => _DetailFState();
}

class _DetailFState extends State<DetailF> {
  double rating = 0;
...
Future myComments() async {
  ...
  Future addLike() async {
    var url = Uri.parse('http://192.168.1.104/gradpro/addLikesFarm.php');
    var response = await http.post(url, body: {
      "user_id": widget.login_id,
      "post_id": widget.post_id,
      "cat_type": widget.cat_type,
 ...
 Future addComments() async {
    var url = Uri.parse('http://192.168.1.104/gradpro/addFarmComments.php');
    var response = await http.post(url, body: {
